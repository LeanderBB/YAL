...
the same identifier declared here:\n",
    ...
    DeclVar*
    StageDecls::resolveVarRef(const TokenInfo& ti) {
        Identifier id(ti.tokenStr);
        DeclBase* scopeDecl = m_activeScope->getDecl(id, true);
        if (scopeDecl == nullptr) {
            Log& log = m_compiler.getLog();
            PrettyPrint::SourceErrorPrint(m_sourceItem,
                                          log,
                                          ti.lineStart,
                                          ...
                                          ti.columnStart,
                                          ti.columnEnd);
            log.error("Variable '%' does not exist in current scope \n",
                      id.getAsString());
            return nullptr;
        }

        if (!scopeDecl->isVariableDecl()) {
            Log& log = m_compiler.getLog();
            PrettyPrint::SourceErrorPrint(m_sourceItem,
                                          log,
                                          ti.lineStart,
                                          ...
                                          ti.columnStart,
                                          ti.columnEnd);
            log.error("'%' is not a variable type.\n",
                      id.getAsString());
            return nullptr;
        }

        return static_cast<DeclVar*>(scopeDecl);
    }

    DeclVar*
    StageDecls::resolveVarRefSelf(const TokenInfo& ti) {
        if (m_activeScope->getKind() != DeclScope::Kind::TypeFunction) {
            Log& log = m_compiler.getLog();
            PrettyPrint::SourceErrorPrint(m_sourceItem,
                                          log,
                                          ti.lineStart,
                                          ...
                                          ti.columnStart,
                                          ti.columnEnd);
            log.error("'self' is not a available outside type functions.\n");
            return nullptr;
        }

        Identifier id(ti.tokenStr);
        DeclBase* decl = m_activeScope->getDecl(id, true);
        YAL_ASSERT(decl != nullptr);
        DeclParamVarSelf* declSelf = dyn_cast<DeclParamVarSelf>(decl);
        YAL_ASSERT(declSelf != nullptr);
        return declSelf;
    }
    ...
    StageDecls::onDuplicateDecl(const DeclBase* oldDecl,
        ...
        log.error("Can not declare '%', existing decl arleady exists here:\n",
                  newDecl->getIdentifier().getAsString());
        PrettyPrint::SourceErrorPrint(log,
